...
public interface APIBackendVisitasService {
        ...
        @DELETE("visits/{id}")
        Call<ResponseBody> deleteVisita(@Path("id") String id);

        @GET("matchingvisits")
        Call<MatchingVisits> getMatchingVisits(@Query("idFacebook") String idFacebook, @Query("idPlace") String idPlace, @Query("date") String date, @Query("startHour") int startHour, @Query("endHour") int endHour);
}
